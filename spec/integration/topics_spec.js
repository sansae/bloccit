...
describe("routes : topics", () => {
  ...
  describe("admin user performing CRUD actions for Topic", () => {
    /* before each test in admin user context, send an authentication request to a route we will create to mock an authentication request */
    beforeEach((done) => {
      User.create({
  ...
  describe("member user performing CRUD actions for Topic", () => {
    ...
    describe("POST /topics/create", () => {
      ...
      it("should not create a new topic", (done) => {
        request.post(options, (err, res, body) => {
          Topic.findOne({
            where: { title: "blink-182 songs" }
          })
          .then((topic) => {
            // no topic should be returned
            expect(topic).toBeNull();
            done();
          })
          .catch((err) => {
